      ...
      <div className="space-y-4">
        {activeTab === 'content' && article.sentences.map((s, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex gap-4">
            ...
            <button onClick={() => handleTTS(s, `s-${i}`)} className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${playingId === `s-${i}` ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-400'}`}>
          ...
          <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 flex items-center justify-between shadow-sm">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-[10px] font-black text-indigo-500 uppercase transition-opacity ${showReadings ? 'opacity-100' : 'opacity-0'}`}>[{v.reading}]</span>
              </div>

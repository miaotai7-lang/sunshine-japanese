    ...
    <div className={`pb-24 animate-fadeIn px-2 ${!showReadings ? 'hide-readings' : ''}`}>
      <div className="flex justify-between items-center py-4 sticky top-0 bg-slate-50/90 backdrop-blur-md z-30">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowReadings(!showReadings)} className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${showReadings ? 'bg-purple-600 text-white shadow-lg' : 'bg-slate-200 text-slate-400'}`}>
      ...
      <header className="mb-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm text-center relative mx-1">
        <button 
        ...
        <div className="text-xl font-bold leading-relaxed mb-4 Japanese-text">{renderSegments(verse.japaneseSegments)}</div>
        {showTranslation && <p className="text-sm text-slate-400 font-medium italic border-t pt-4">"{verse.chineseTranslation}"</p>}
      </header>

      <nav className="flex gap-2 p-1.5 bg-slate-200/50 rounded-3xl mb-6 mx-1">
      ...
      <div className="space-y-4 px-1">
        {activeTab === 'content' && (
          Array.isArray(verse.sentences) && verse.sentences.length > 0 ? (
            verse.sentences.map((segments, i) => (
              <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 flex flex-col gap-4 shadow-sm">
                <div className="flex-1">
                  ...
                  {showTranslation && verse.translations?.[i] && (
                    <p className="text-slate-400 text-sm mt-4 border-l-4 border-purple-100 pl-4 font-medium leading-relaxed">{verse.translations[i]}</p>
                  )}
                </div>
                <div className="flex justify-end gap-3 mt-1">
                  ...
                  <button onClick={() => handleTTS(segments, `s-${i}`)} className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${playingId === `s-${i}` ? 'bg-purple-600 text-white shadow-lg' : 'bg-purple-50 text-purple-600'}`}>
            ...
            <div className="bg-white p-10 rounded-3xl text-center text-slate-400 text-xs italic">AI 正在努力生成正文解析...</div>
          )
        )}

        {activeTab === 'vocab' && (
          Array.isArray(verse.vocabulary) && verse.vocabulary.length > 0 ? (
              ...
              <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 flex items-center justify-between shadow-sm">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-black text-purple-500">[{v.reading}]</span>
                  </div>
            ...
            <div className="bg-white p-10 rounded-3xl text-center text-slate-400 text-xs italic">未发现重点词汇解析。</div>
          )
        )}

        {activeTab === 'grammar' && (
          Array.isArray(verse.grammar) && verse.grammar.length > 0 ? (
            verse.grammar.map((g, i) => (
            ...
            <div className="bg-white p-10 rounded-3xl text-center text-slate-400 text-xs italic">未发现重点语法解析。</div>
          )
        )}
      </div>
    </div>
